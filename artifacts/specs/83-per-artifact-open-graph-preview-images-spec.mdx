...
> **Implementation note (pivot, approved 2026-06-14):** the renderer is **Python**
> `scripts/gen-og-images.py` run via `uv run --with playwright python3` — NOT a node
> `.mjs`. Rationale: `$FORGE_DIR` (where `build.sh` runs) has no `node_modules` and the
> build is 100% Python; `playwright` npm lives only in dev (`.claude/fc-loop/`). `uv`
> (0.11.1) + cached chromium (`~/.cache/ms-playwright`) make the Python path work with
> zero new runtime deps (verified: chromium launch + 1200×630 screenshot). Throughout
> this spec, read the node phrasing as its Python equivalent: `.mjs`→`.py`,
> `newContext`→`new_context`, `console.warn`+`process.exit(0)`→`print`+`sys.exit(0)`,
> `fullPage:false`→`full_page=False`, `node <script>`→`uv run --with playwright python3 <script>`.
...
1. `build.sh` runs `uv run --with playwright python3 scripts/gen-og-images.py` before `gen-og-tags.py` (guarded: `uv` absent → skip + warn, never abort).
2. `gen-og-images.py` uses its **own** Playwright setup (it does **not** invoke
...
6. `build.sh` rsync ships `.og.png` into `_dist/` (PNG not excluded; ~0.2–0.8 MB each, far
   ...
   `gen-og-images.py` reaches `$FORGE_DIR/scripts/` on the runtime host.
...
```mermaid
flowchart LR
    H[artifact .html] --> G[gen-og-images.py]
...
|---|---|---|---|
| `gen-og-images.py` | htmlPath, htmlMtime, pngMtime | build, before tags | this issue |
...
|---|---|---|---|
| S1 | `node scripts/gen-og-images.py` | own Playwright setup (viewport 1200×630, `fullPage:false`, dSF 2); walk `$FORGE_DIR/**/*.html` (gen-og-tags exclusions) → render stale → write `<name>.og.png.tmp` → atomic rename | htmlPath, mtimes |
...
| S6 | `plugins/forge/Makefile` deploy | add `cp -v $(REPO_ROOT)/scripts/gen-og-images.py $(FORGE_DIR)/scripts/gen-og-images.py` | — |
...
| # | Slice | Demo |
|---|---|---|
| 1 | `gen-og-images.py` renders one artifact → 1200×630 `.og.png` (own Playwright setup; mtime-idempotent; tmp+rename; graceful chromium-fail + per-artifact skip) | run on one artifact, inspect PNG dims + re-run no-op + simulate fail leaves no partial |
...
- [ ] `gen-og-images.py` produces a 1200×630 PNG sibling for a given artifact HTML (own Playwright setup, `fullPage:false`).
...
- [ ] `plugins/forge/Makefile` `deploy` copies `gen-og-images.py` to `$FORGE_DIR/scripts/`.
