...
- **Skeleton:**
  ...
  - `async function initMermaid()` — scoped `document.querySelectorAll('.slide--diagram [data-mermaid]')`, `import()` mermaid ESM, `mermaid.initialize({ startOnLoad: false })`, ∀ node: `const id = 'mermaid-slide-' + i; const { svg } = await mermaid.render(id, node.textContent); node.innerHTML = svg;`.
  ...
  - Bootstrap: `document.addEventListener('DOMContentLoaded', async () => { autoFit(); await initMermaid(); new SlideEngine(document.querySelector('.deck')); });`.
