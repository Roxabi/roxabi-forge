# roxabi-forge — agent context

## Mission

**forge.roxabi.dev** = team HTML artifact host (decks, talks, guides).

| Host | Job |
|---|---|
| `forge.roxabi.dev` | Internal artifacts + keyed share links |
...
```bash
S=plugins/roxabi-forge/scripts/publish.sh
...
Deploy: `publish.sh` → `wrangler pages deploy` (token in `~/.config/roxabi/forge/forge.env`). HTML **not in git**.
...
plugins/roxabi-forge/ # publish + setup — NOT HTML craft
...
# craft (slides / pages / diagrams) = external tools, not this repo
```

## Machine config (artifacts in the hub)

**SSOT HTML** = shared hub (path **differs** per person)  
**Deploy** = `publish.sh` → build from hub → `wrangler pages deploy`  
**main** = engine only (no HTML)

| File | Role |
|---|---|
| `~/.config/roxabi/forge/forge.config.json` | local: `hub_root`, `pages_project`, `public_host`, `vault_markers` — **not git** |
| `~/.config/roxabi/forge/forge.env` | credentials + the Access/Shlink Pages vars: CF token · account · KV id · `CF_ACCESS_*` · `SHLINK_API_URL` (chmod 600, dir 700) — **not git**, and never `public_host` / `pages_project` |
...
plugins/roxabi-forge/scripts/forge-discover.sh --write   # attach to an existing forge
plugins/roxabi-forge/scripts/forge-provision.sh          # OR stand one up on an empty account (interactive only)
plugins/roxabi-forge/scripts/forge-doctor.sh
plugins/roxabi-forge/scripts/publish.sh --rebuild-index  # hub → wrangler Pages
```

`--write` merges the discovered keys into `forge.env` (prints **key names only**) and persists the confirmed `pages_project` into an **existing** `forge.config.json` — it never creates that file. Never discoverable: `CLOUDFLARE_API_TOKEN` (API token permissions: Pages Edit · Workers KV Storage Edit · Account Settings Read · **Browser Run · Edit** — the dashboard group name, not the OAuth scopes above) and `hub_root` (local vault path). Both come from the operator via **`/roxabi-forge-setup`**.
...
Project-name defaults are two, on purpose: `forge-discover.sh` → `roxabi-forge` (the Roxabi forge), `forge-provision.sh` prompt → `[forge]` (never brand a client's project). `forge.config.json`'s `pages_project` overrides both.
...
`publish.sh` refuses to deploy while doctor reports `ok: false`: it dies naming `/roxabi-forge-setup` instead of falling back to the example config. `--share <slug>` verifies the hub artifact exists before any clone or deploy.

## Plugin in this repo

| Plugin | Contents |
|---|---|
| **`roxabi-forge`** | `roxabi-forge-publish` · `roxabi-forge-setup` — **Cloudflare upload only** |

Slide engine + diagrams → external plugins (see **roxabi-forge-setup**).  
**Rocky**: `rocky@rocky` (`Roxabi/rocky`) — outside this repo.
...
/plugin marketplace add Roxabi/rocky
/plugin install rocky@rocky
```

## Docs

- `docs/cloudflare-access.md` — Access + Bypass `/s` — **Roxabi team-prod instance (forge.roxabi.dev)**
- `docs/cloudflare-pages.md` — Pages deploy + env vars — **Roxabi team-prod instance (forge.roxabi.dev)**
...
A forge on someone else's Cloudflare account: `forge-provision.sh` + `"vault_markers": []` — see `docs/artifacts-config.md`, not the two Roxabi-instance docs above.
...
```bash
plugins/roxabi-forge/scripts/gen-og-images.sh
plugins/roxabi-forge/scripts/gen-og-images.sh --slug my-slug --force --quality 80
```
...
POSTs to, with `cacheTTL=0` so a repeat cannot replay the previous verdict.
It surfaces as `browser_run: permission ok` among the online checks, or as one
...
best-effort on thumbnails. Skipped, with no network call, when the token or
the account id is missing (already a deploy blocker) and when the rest of the
live pass failed (the sentence would be false).

`permission ok` is the exact claim: the credential may render. It does not
prove a 6.5 MiB payload renders, that the plan's Quick Actions rate allows 33
of them, or that the permission still exists at publish time. Design
discussion and measurements: PR #55.
...
4. Publish = hub SSOT + `wrangler pages deploy`; CF token = `~/.config/roxabi/forge/forge.env` — never in git / GH
...
9. Missing forge config → tell the operator to run `/roxabi-forge-setup` (do not invent hub_root; do not auto-invoke roxabi-forge-setup)
...
12. `plugins/roxabi-forge/scripts/` is **English-only** (CLI output, `die`/`info`/`warn`, doctor lines, comments, docstrings); French-by-design allowlist = `gen-index.py` (site UI), `hub-index.py` (vault note), `share-bar.js` (team toolbar) — enforced by `tests/python/test_lang_boundary.py`, so a French string outside the allowlist fails CI
