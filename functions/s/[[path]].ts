...
export const onRequest: PagesFunction<ForgeEnv> = async (context) => {
  ...
  const exchange = url.pathname.match(/^\/s\/(.+\.html)\/([^/]+)\/?$/)
  if (exchange && !exchange[1].includes("..")) {
    const page = exchange[1]
    const key = exchange[2]
    const vis = await getVisibility(context.env.SHARES, page)
    ...
    headers.set(
      "set-cookie",
      `forge_share=${encodeURIComponent(`${page}|${key}`)}; HttpOnly; Secure; SameSite=Lax; Path=/`,
    )
