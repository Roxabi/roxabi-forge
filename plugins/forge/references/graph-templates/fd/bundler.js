...
export function buildEngine(fdDir, type) {
  ...
  // __fd entry: minimal window exposure so a separate bootstrap <script> can call
  // window.__fd.run(descriptor, canvasEl, svgEl) without knowing IIFE internals.
  // initEngine, renderNodes, draw, wireResize are all IIFE-scoped; without this
  // a cross-script bootstrap silently fails (functions not on window).
  const fdEntry = [
  ...
  // Wrap in IIFE to avoid polluting the global scope (except for window.__fd + window.__fdTypes)
  return `(function(){\n'use strict'\n\n${body}\n\n${fdEntry}\n})()`
}
