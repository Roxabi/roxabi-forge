...
# roxabi-forge-setup — machine config for roxabi-forge

Configure the **local hub path** (differs per person) and the central
artifacts folder. Without this config, roxabi-forge-publish stops and asks you to
...
Relative paths like `../hub` are **not** portable → always an **absolute**
path in the local config.

`pages_project` and `public_host` SSOT is `~/.config/roxabi/forge/forge.config.json`.
...
```
✅ ~/.config/roxabi/forge/forge.config.json (merged from example; this skill creates it)
✅ hub_root = valid vault per vault_markers
✅ $hub_root/$artifacts_dir exists
✅ ~/.config/roxabi/forge/forge.env (discover --write fills account + KV + Access; token still required, chmod 600)
...
# in-repo:
# S=plugins/roxabi-forge/scripts/forge-doctor.sh
bash "$S"
...
`forge.env` path, default `~/.config/roxabi/forge/forge.env`):
...
`CF_ACCESS_AUD`, `forge_env_permissions`, `forge_repo`, `refused_target`. An
empty `public_host`, and a `public_host` / `pages_project` listed in
`refused_targets`, are also config **issues**: they make doctor exit **1**, so
no arrow is printed for them — fix them in step 2 / 5a.
...
in the config (a hub without marker directories uses `[]` — the folder must
...
mkdir -p ~/.config/roxabi/forge
chmod 700 ~/.config/roxabi/forge

EXAMPLE="$FORGE_ROOT/forge.config.example.json"
# in-repo: plugins/roxabi-forge/forge.config.example.json
LOCAL=~/.config/roxabi/forge/forge.config.json
...
HUB="$HUB" ENGINE="${ENGINE:-}" EXAMPLE="$EXAMPLE" LOCAL="$LOCAL" python3 - <<'PY'
import json
import os
import sys
...
```bash
if [ ! -f ~/.config/roxabi/forge/hub-root ]; then
  echo "$HUB" > ~/.config/roxabi/forge/hub-root
fi
```
...
- Live publish: skill `roxabi-forge-publish` → wrangler Pages (not git)
...
| `1` | Hub still KO. **Stop.** Name the issues. Do not invent a path. Operator reruns `/roxabi-forge-setup`. |
...
One question: is this the **shared Roxabi** Cloudflare account, or the
**client's own** account?

**Shared Roxabi** — keep `public_host` and `pages_project` from the example
config (`forge.roxabi.dev` / `forge`). Continue 5b with no `--project` unless
doctor/discover already named a different one.

**Client's own account** — do **not** inherit `forge.roxabi.dev` /
`roxabi-forge`. Ask (one at a time) for the public host (no `https://`) and
...
if not local.is_file():
    ...
    print("→ rerun roxabi-forge-setup from step 1", file=sys.stderr)
    sys.exit(1)
...
| `cannot write` | the config's directory or the file is read-only | Fix the permissions on `~/.config/roxabi/forge` (`chmod 700`) and the file, then `--write` |
| `cannot create` `<dir>` | `~/.config/roxabi/forge` cannot be created | `mkdir -p ~/.config/roxabi/forge && chmod 700 ~/.config/roxabi/forge`, then `--write` |
| `could not be chmod 600` | credentials were written but are not private | `chmod 600 ~/.config/roxabi/forge/forge.env` **now**, then `--write` to verify |
...
Merges into `~/.config/roxabi/forge/forge.env` (chmod 600). Prints **key names only**.
...
`pages_project` into an **existing** `~/.config/roxabi/forge/forge.config.json` (never
...
```bash
# ~/.config/roxabi/forge/forge.env (chmod 600)
...
Local key file for syncing into Pages (optional): `~/.config/roxabi/forge/shlink-api-key` (chmod 600).
Never commit the key or put it in `forge.config.json`.

```bash
: "${FORGE_ROOT:?run § Shell setup first}"
set -a; source ~/.config/roxabi/forge/forge.env; set +a
...
`--export-env`), not a hardcoded `roxabi-forge`.
...
Not part of `roxabi-forge`, which only uploads HTML. **Install** (user scope)
whatever produces your artifacts — hub doctor stays OK without them.

| Repo | Install |
|---|---|
| [frontend-slides](https://github.com/zarazhangrui/frontend-slides) | slide decks · live host = `roxabi-forge-publish`, **never Vercel** |
...
```
## roxabi-forge-setup — report

**config**     : ~/.config/roxabi/forge/forge.config.json
**hub_root**   : [path OK]
...
**Next**
- Generate: any craft plugin (step 7) → write under $artifacts/<slug>/
- Publish: roxabi-forge-publish (hub → wrangler Pages)
...
2. `~/.config/roxabi/forge/forge.config.json`
3. else plugin **`forge.config.example.json`** (defaults; empty `hub_root` → doctor exit 1)

Empty `hub_root` may still bootstrap from `HUB_ROOT` env or `~/.config/roxabi/forge/hub-root`, but **doctor requires** a valid vault.
