...
FORGE_REPO="${_ENV_FORGE_REPO:-${FORGE_REPO:-https://github.com/Roxabi/roxabi-forge.git}}"
SHLINK_DOMAIN="${_ENV_SHLINK_DOMAIN:-${FORGE_SHLINK_DOMAIN:-${SHLINK_DOMAIN:-}}}"
...
PUBLIC_HOST="${FORGE_PUBLIC_HOST:-forge.roxabi.dev}"
PAGES_PROJECT="${FORGE_PAGES_PROJECT:-forge}"
...
sys.exit(1)'); then
  ...
  die "forge config incomplete — run the roxabi-forge-setup skill (forge-doctor.sh for the full report)"
}
...
Usage:
  ...
  --tree    : publish the whole data tree as one snapshot, each page at its
              own path. The share bar is injected into the deploy copy only.
              OG previews are rendered at publish into the SSOT
              (<page>.og.png beside the page — that is content, and it is
              where the catalogue looks). The renderer is og_renderer in
              the forge config: browser-run, playwright, or off.

  --dry-run : accepted anywhere in argv, for every command — builds and
              validates everything (engine, hub snapshot, wrangler.toml)
              without deploying and without mutating KV. A tree dry run
              also skips writing OG previews.

  --force-og : re-render every thumbnail. A slug publish passes this to
              gen-og-images.sh --force; --tree passes it to og_tree.py --force.
...
SCRIPTS() {
  ...
  if [ -n "${WORK:-}" ] && [ -d "$WORK/repo/plugins/roxabi-forge/scripts" ]; then
    echo "$WORK/repo/plugins/roxabi-forge/scripts"
  else
    echo "$SCRIPT_DIR"
  fi
}
...
share_bar_script() {
  ...
  [ -f "$p" ] || die "inject-share-bar.py missing — reinstall the roxabi-forge plugin, then forge-doctor.sh"
  echo "$p"
}
...
build_from_hub() {
  ...
  [ -f "$build_py" ] || die "build-site-from-hub.py missing — reinstall the roxabi-forge plugin, then forge-doctor.sh"
...
# Load token + account from ~/.config/roxabi/forge/forge.env (never print values)
forge_env_mode() {
  local f="${FORGE_ENV_FILE:-$HOME/.config/roxabi/forge/forge.env}"
  [ -f "$f" ] || return 0
...
if blockers:
  ...
  echo "  → CLOUDFLARE_API_TOKEN goes in ~/.config/roxabi/forge/forge.env (chmod 600)" >&2
  echo "  → /roxabi-forge-setup                 # rebuild the whole local install" >&2
...
patch_wrangler_for_deploy() {
  ...
  [ -n "$kv" ] || die \
    "FORGE_SHARES_KV_ID missing — set in ~/.config/roxabi/forge/forge.env (see .env.example)"
  [ -n "$team" ] || die \
    "CF_ACCESS_TEAM_DOMAIN missing — set in ~/.config/roxabi/forge/forge.env (see .env.example)"
  [ -n "$aud" ] || die \
    "CF_ACCESS_AUD missing — set in ~/.config/roxabi/forge/forge.env (see .env.example)"
  [ -n "$host" ] || die \
...
deploy_pages() {
  ...
  [ -n "${CLOUDFLARE_API_TOKEN:-}" ] || die \
    "CLOUDFLARE_API_TOKEN not in the publish environment — put it in ~/.config/roxabi/forge/forge.env (chmod 600), then forge-doctor.sh"
  local acct="${CLOUDFLARE_ACCOUNT_ID:-}"
  [ -n "$acct" ] || die \
    "CLOUDFLARE_ACCOUNT_ID not in the publish environment — add it to ~/.config/roxabi/forge/forge.env (forge-discover.sh prints it), then forge-doctor.sh"
...
write_source_to_hub() {
  local slug="$1"
  [ -n "${ARTIFACTS_ROOT:-}" ] || die "ARTIFACTS_ROOT is empty — set artifacts_dir in ~/.config/roxabi/forge/forge.config.json, then run forge-doctor.sh"
...
# Copy hub SSOT to Google Drive so the next rclone pull (sync --delete)
# cannot eat local-only og.jpg / HTML. Best-effort: Drive down must not
# fail the Pages deploy. Operator layout: hub_root/../scripts/push-hub.sh
hub_drive_push() {
  $DRY_RUN && return 0
  local hub="${FORGE_HUB_ROOT:-}"
  local art="${ARTIFACTS_ROOT:-}"
  [ -n "$hub" ] && [ -d "$hub" ] || return 0
  [ -n "$art" ] && [ -d "$art" ] || return 0
  local pusher
  pusher="$(cd "$hub/.." && pwd)/scripts/push-hub.sh"
  if [ ! -x "$pusher" ]; then
    warn "hub Drive push skipped — $pusher not executable"
    return 0
  fi
  local rel="${art#"$hub"/}"
  if [ "$rel" = "$art" ]; then
    warn "hub Drive push skipped — artifacts not under hub_root"
    return 0
  fi
  local paths=() slug="${1-}"
  if [ -n "$slug" ]; then
    paths+=("${rel}/${slug}")
    local note
    note="$(dirname "$rel")/${slug}.md"
    [ -f "${hub}/${note}" ] && paths+=("$note")
  else
    paths+=("$rel")
  fi
  local rc=0
  # Bounded retry: the counter only caps attempts, its value is never read.
  for _ in 1 2 3 4 5; do
    if "$pusher" ${paths[@]+"${paths[@]}"}; then
      ok "hub Drive push"
      return 0
    fi
    rc=$?
  ...
  warn "hub Drive push failed (rc=$rc) — freeze pull-watch until retry"
}
...
cmd_list() {
  ...
  if [ -z "${ARTIFACTS_ROOT:-}" ] || [ ! -d "$ARTIFACTS_ROOT" ]; then
    die "ARTIFACTS_ROOT missing — roxabi-forge-setup"
  fi
  local d
...
# Tree publish: the whole data tree becomes the deploy snapshot, at the same
# relative paths. This is the Roxabi shape — a page is identified by its path,
# not by a slug directory.
#
# The share bar is injected into the copy under $WORK/repo/site and never into
# the SSOT, so two machines syncing the same tree never fight over a bar. OG
# previews are the exception, and only on a real publish: they are content,
# written beside the page by render_tree_og before the snapshot is built,
# because thumb_for() reads <page>.og.png from the SSOT. A dry run still
# writes nothing, which is why this command needs no hub sandbox.
#
# The drift guard still runs (preflight_before_live): a tree missing a page the
# live record holds is the same full-snapshot deletion risk as a missing slug,
# and snapshot.py now fingerprints page paths under `layout: "tree"`.
render_tree_og() {
  # Order is load-bearing: the snapshot copies <page>.og.png into the deploy
  # and builds the card thumb from it. Rendering after build_tree_from_ssot
  # would publish yesterday's card. A missing thumbnail degrades one page, so
  # this function warns and returns — it must not die.
  local renderer="${FORGE_OG_RENDERER:-browser-run}"
  local py summary rc line
  py="$(SCRIPTS)/og_tree.py"
  [ -f "$py" ] || py="$SCRIPT_DIR/og_tree.py"
  # $(SCRIPTS) is the engine archive about to deploy. og_tree.py may not be
  # in that archive yet (a publish from a dirty checkout); the installed
  # plugin copy is the fallback, same resolution as tree_snapshot.py.
  if [ ! -f "$py" ]; then
    warn "og_tree.py missing — OG previews skipped (publish continues)"
    return 0
  fi
  if [ -z "${ARTIFACTS_ROOT:-}" ] || [ ! -d "$ARTIFACTS_ROOT" ]; then
    warn "artifacts root missing — OG previews skipped (publish continues)"
    return 0
  fi
  if $DRY_RUN; then
    info "dry run — would render OG previews ($ARTIFACTS_ROOT, renderer=$renderer); SSOT not written"
  else
    info "OG previews ($renderer) → $ARTIFACTS_ROOT"
  fi
  local args=(--source "$ARTIFACTS_ROOT" --renderer "$renderer")
  if $FORCE_OG; then
    args+=(--force)
  fi
  if $DRY_RUN; then
  ...
  rc=0
  summary="$(python3 "$py" ${args[@]+"${args[@]}"} 2>&1)" || rc=$?
  while IFS= read -r line; do
    [ -n "$line" ] || continue
    case "$line" in
      og:*) info "$line" ;;
  ...
  if [ "$rc" -ne 0 ]; then
    warn "OG preview step exited $rc (publish continues)"
  fi
  return 0
}

build_tree_from_ssot() {
  local builder
  builder="$(SCRIPTS)/tree_snapshot.py"
  [ -f "$builder" ] || builder="$SCRIPT_DIR/tree_snapshot.py"
  [ -f "$builder" ] || die "tree_snapshot.py missing — reinstall the forge plugin, then forge-doctor.sh"
  [ -n "${ARTIFACTS_ROOT:-}" ] || die "artifacts root unresolved — set hub_root and artifacts_dir in the forge config, then forge-doctor.sh"
  [ -d "$ARTIFACTS_ROOT" ] || die "artifacts root missing: $ARTIFACTS_ROOT"
  info "tree snapshot $ARTIFACTS_ROOT → $WORK/repo/site"
  python3 "$builder" --source "$ARTIFACTS_ROOT" --dest "$WORK/repo/site" >/dev/null \
    || die "tree snapshot failed — check $ARTIFACTS_ROOT"
}

# site/index.html is generated, so it is gitignored and absent from the engine
# archive. Without this the tree deploy ships every page but no landing.
build_landing_shell() {
  local gen
  gen="$(SCRIPTS)/gen-index.py"
  [ -f "$gen" ] || gen="$SCRIPT_DIR/gen-index.py"
  [ -f "$gen" ] || die "gen-index.py missing — reinstall the forge plugin, then forge-doctor.sh"
  info "landing shell → $WORK/repo/site/index.html"
  python3 "$gen" --shell-only "$WORK/repo/site" >/dev/null \
    || die "landing shell generation failed"
  [ -s "$WORK/repo/site/index.html" ] || die "landing shell is empty — gen-index.py produced nothing"
}
...
cmd_publish_tree() {
  ...
  require_forge_config
  acquire_publish_lock "_tree"
  source_cf_credentials
  preflight_before_live

  WORK="$(mktemp -d)"
  materialize_engine
  # Before the snapshot: thumb_for() reads the PNG this step writes.
  # `|| warn` so a shell error here cannot become a failed publish.
  render_tree_og || warn "OG preview step failed (publish continues)"
  build_tree_from_ssot
  build_landing_shell

  if deploy_pages; then
    ok "published tree snapshot (share bar stays out of the SSOT)"
    echo "  Team: https://${PUBLIC_HOST}/  (Access)"
  fi
}
