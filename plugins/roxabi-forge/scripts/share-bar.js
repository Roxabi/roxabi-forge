 ...
 * slug: chemin de page de l'arbre (`lyra/visuals/architecture.html`), ou slug
 * hérité déduit de `/a/<slug>/`. Les deux nomment la même clé KV.
 */
(function () {
  ...
  if (!slug) {
    ...
    if (m) {
      ...
      /* Layout par arbre : la page EST son chemin. Pages sert la forme sans
       * extension, l'enregistrement est tenu sur la forme .html — on recolle
       * l'extension quand le dernier segment n'en porte pas. Un répertoire
       * peut contenir un point, donc on ne teste que ce dernier segment. */
      var p = location.pathname.replace(/^\/+/, "");
      var last = p.slice(p.lastIndexOf("/") + 1);
      if (last) slug = /\.html$/.test(last) ? p : last.indexOf(".") === -1 ? p + ".html" : "";
    }
  }
  /* Plus de garde sur `/a/` : la barre est injectée dans les pages, et c'est
   * l'API — seule à savoir qui est l'équipe — qui décide de son affichage. */
  if (!slug) return;
  ...
  function pageUrl() {
    /* Arbre : la page est servie sur son chemin, dans sa forme canonique sans
     * extension (Pages redirige `.html` en 308). Slug hérité : sous `/a/`. */
    if (/\.html$/.test(slug)) return location.origin + "/" + slug.replace(/\.html$/, "");
