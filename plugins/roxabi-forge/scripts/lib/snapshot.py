...
def _page_digests(root: Path) -> dict[str, str]:
    scripts = Path(__file__).resolve().parents[1]
    if str(scripts) not in sys.path:
        sys.path.insert(0, str(scripts))
    from tree_snapshot import page_digests

    return page_digests(root)


def fingerprint(root: Path, layout: str = "") -> dict[str, str]:
    """{page: sha256hex}. `layout` is the config's, "" falls back to the tree shape.

    The Roxabi forge keys a page on its path, so `layout: "tree"` hashes every
    HTML file in place. A tree whose top level happens to hold an index.html
    would otherwise be mistaken for the legacy slug layout, and the guard would
    then compare a handful of directory digests against a record of page paths.
    """
    if layout == "tree":
        return _page_digests(root)
    if layout == "slug":
        return {d.name: slug_digest(d) for d in slug_dirs(root)}
    slugs = slug_dirs(root)
    if slugs:
        return {d.name: slug_digest(d) for d in slugs}
    return _page_digests(root)
...
def _local_fingerprint() -> tuple[dict[str, str], Path | None, str]:
    ...
    if root is None:
        return {}, None, "artifacts root unresolved: set hub_root and artifacts_dir (roxabi-forge-setup)"
    if not root.is_dir():
        return {}, root, f"artifacts root missing: {root} -- roxabi-forge-setup?"
    try:
        return fingerprint(root, str(cfg.get("layout") or "")), root, ""
