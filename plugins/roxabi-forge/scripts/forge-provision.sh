...
ENV_FILE="${FORGE_ENV:-$HOME/.config/roxabi/forge/forge.env}"
CONFIG_FILE="${FORGE_CONFIG:-$HOME/.config/roxabi/forge/forge.config.json}"
...
banner "roxabi-forge — provision a forge on your Cloudflare account"
...
note "Browser Run · Edit renders the OG thumbnails."
note "Not fatal: without it the deploy still works, every render fails per slug."
...
if [ -n "$_HUB_SEED" ]; then
  ...
  ask FORGE_HUB_ROOT "Absolute path to the folder that will hold artifacts (e.g. $HOME/forge-hub):"
fi
case "$FORGE_HUB_ROOT" in /*) ;; *) fail "an absolute path is required, e.g. $HOME/forge-hub" ;; esac
...
FORGE_PAGES_PROJECT="$FORGE_PAGES_PROJECT" FORGE_LIB="$LIB_DIR" python3 - <<'PY'
import json, os, sys
from pathlib import Path
sys.path.insert(0, os.environ["FORGE_LIB"])
from load_config import pick_forge_repo
...
# is a policy the operator may already have chosen (a notes vault has its own
...
print(f"  ✓ wrote {p}")
print(f"  ✓ forge_repo = {cfg['forge_repo']}")
PY
...
case "$DOCTOR_RC" in
  ...
  *) warn "Doctor: the local config or hub is broken (exit $DOCTOR_RC) — fix the issues above, or re-run /roxabi-forge-setup." ;;
esac
say ""
say "Smoke tests:"
