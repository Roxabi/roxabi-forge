...
Usage:
  build-site-from-hub.py --repo-root /path/to/roxabi-forge
...
def build(
    ...
    if art is None or not art.is_dir():
        print(f"✗ artifacts root missing: {art} — roxabi-forge-setup?", file=sys.stderr)
        return 1
    ...
    if not slugs:
        from tree_snapshot import build_tree_snapshot, page_digests

        pages = page_digests(art)
        if not pages:
            print(f"✗ no artifacts with index.html under {art}", file=sys.stderr)
            return 1
        if dry_run:
            print(f"would snapshot {len(pages)} tree page(s) from {art}")
            return 0
        build_tree_snapshot(art, site_dir)
        print(f"✓ built {len(pages)} tree page(s) from {art}")
        return 0
    ...
    if not gen.is_file():
        gen = repo_root / "plugins/roxabi-forge/scripts/gen-index.py"
    if gen.is_file():
        ...
        if r.returncode != 0:
            ...
            gen_in_repo = repo_root / "plugins/roxabi-forge/scripts/gen-index.py"
...
def main(argv: list[str]) -> int:
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="roxabi-forge root (default: parents of this script)",
    )
    ...
    if args.repo_root:
        ...
        # plugins/roxabi-forge/scripts → repo root
