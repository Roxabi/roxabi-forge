...
SSOT HTML: hub artifacts/ (forge.config). Deploy copy: roxabi-forge git site/a/.
Hub notes:
...
Hub root resolution:
  ...
  3) HUB_ROOT env / ~/.config/roxabi/forge/hub-root (legacy bootstrap)
"""
...
def resolve_hub(explicit: str | None) -> Path:
    ...
    cfg_file = Path.home() / ".config/roxabi/forge/hub-root"
    if cfg_file.is_file():
        return Path(cfg_file.read_text(encoding="utf-8").strip()).expanduser().resolve()
    return (Path.home() / ".roxabi").resolve()
...
def write_slug_note(hub: Path, item: dict, host: str) -> Path:
    ...
    lines = [
        ...
        f"- Deploy live : repo `Roxabi/roxabi-forge` → `site/a/{slug}/`",
        f"- Registry : `registry/{slug}.json`",
        "",
        "Éditer le HTML dans le hub ; republier via roxabi-forge-publish pour le live.",
        "",
    ]
...
def write_catalogue(hub: Path, items: list[dict], host: str) -> Path:
    ...
    lines = [
        ...
        f"> SSOT HTML : [Roxabi/roxabi-forge](https://github.com/Roxabi/roxabi-forge)",
        "",
...
def main() -> int:
    ...
    ap.add_argument("--host", default=os.environ.get("PUBLIC_HOST", "forge.roxabi.dev"))
