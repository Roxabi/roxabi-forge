...
[ -d "$LIB_DIR" ] || die "lib missing: $LIB_DIR — reinstall the roxabi-forge plugin"
...
if not d.get("ok"):
    ...
    print(
        "✗ forge doctor: config KO (%d issue%s) — run the roxabi-forge-setup skill"
        % (n, "" if n == 1 else "s"),
        file=sys.stderr,
    )
    sys.exit(1)
...
print("roxabi-forge · doctor" + (" · online" if online else ""))
...
print(f"  og       : {d.get('og_renderer') or 'browser-run'}")
...
if ok and deploy and online_ok:
    ...
    for w in warnings:
        print(f"  ⚠ {w}")
    print()
    print("→ run the roxabi-forge-setup skill to create/complete the local config.")
...
[ -n "$ENV_PATH" ] || ENV_PATH="$HOME/.config/roxabi/forge/forge.env"
BLOCKERS=${BLOCKER_LINE#blockers:}

# deploy_blockers code → the command that fixes it (load_config.doctor codes).
# No public_host / refused_target case on purpose: both are also config issues
# (an empty public_host, a target listed in refused_targets), so ok is false
# and the run exits 1 before any hint is printed. The *) arm covers them if
# that ever decouples.
blocker_hint() {
  case "$1" in
    token)
