...
#
# --write merges the discovered credentials into ~/.config/roxabi/forge/forge.env
# (chmod 600) and remembers pages_project in an *existing*
# ~/.config/roxabi/forge/forge.config.json — it never creates that file, and it never
...
ENV_PATH="${FORGE_ENV:-$HOME/.config/roxabi/forge/forge.env}"
CONFIG_PATH="${FORGE_CONFIG:-$HOME/.config/roxabi/forge/forge.config.json}"
RELINK="reinstall or relink the plugin (\`omp plugin install roxabi-forge@roxabi-forge\`, or \`omp plugin link ./plugins/roxabi-forge\` from a checkout)"
...
# Default project: forge.config pages_project, else forge.
if [ -z "$PROJECT" ]; then
  ...
  PROJECT="${FORGE_PAGES_PROJECT:-forge}"
fi
...
missing_hint() {
  case "$1" in
    ...
    SHLINK_API_URL)
      echo "only needed for shortlinks — set SHLINK_API_URL in ${ENV_PATH}, then re-deploy with publish.sh" ;;
    *)
...
# config" issue must stay meaningful, and the roxabi-forge-setup skill owns creation.
...
if not path.is_file():
    ...
    print("    → run /roxabi-forge-setup to write it (forge-discover.sh never creates it)")
    raise SystemExit(0)
try:
...
PY
  then
    die "could not remember pages_project in ${CONFIG_PATH} — it must be a writable JSON object; fix it (or re-run /roxabi-forge-setup to rewrite it), then re-run \`forge-discover.sh --write\`"
  fi
}
