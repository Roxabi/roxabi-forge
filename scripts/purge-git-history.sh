...
# (~/.config/roxabi/forge/forge.env → CLOUDFLARE_ACCOUNT_ID, FORGE_SHARES_KV_ID, read
...
LIB_DIR="$ROOT/plugins/roxabi-forge/scripts/lib"
cd "$ROOT"
...
Usage: purge-git-history.sh [--yes] [--dry-run]
  ...
  ~/.config/roxabi/forge/forge.env, never from this file.
USAGE
      exit 0 ;;
...
eval "$(PYTHONPATH="$LIB_DIR${PYTHONPATH:+:$PYTHONPATH}" python3 -c \
  ...
  || die "load_config.py failed — run plugins/roxabi-forge/scripts/forge-doctor.sh"

ACCOUNT_ID="${CLOUDFLARE_ACCOUNT_ID:-}"
KV_ID="${FORGE_SHARES_KV_ID:-}"
ENV_FILE="${FORGE_ENV_FILE:-$HOME/.config/roxabi/forge/forge.env}"
