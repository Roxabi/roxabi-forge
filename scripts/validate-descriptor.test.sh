...
# D) node-graph type missing its nodes array still fails the gate
...
if python3 "$VALIDATOR" --in "$NONODES" --expect "$EXPECT" >/dev/null 2>&1; then
  fail "D: architecture descriptor missing nodes should still fail"
else
  pass "D: architecture descriptor missing nodes fails the gate"
fi
