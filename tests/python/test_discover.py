...
`wrangler whoami` output (captured against the live roxabi-forge project, with
...
class ParsePagesConfigTests(unittest.TestCase):
    def test_reads_production_vars_and_shares_kv(self) -> None:
        ...
        self.assertEqual(parsed["name"], "roxabi-forge")
...
class DiscoveredEnvTests(unittest.TestCase):
    def test_maps_every_discoverable_forge_env_key(self) -> None:
        d = discovered_env(CONFIG, WHOAMI, "roxabi-forge")
        self.assertEqual(d["project"], "roxabi-forge")
    ...
    def test_project_falls_back_to_config_name(self) -> None:
        self.assertEqual(discovered_env(CONFIG, WHOAMI)["project"], "roxabi-forge")
...
PAGES_JSON_LIST = '[{"name": "roxabi-forge"}, {"name": "client-site"}]'
...
"""
    ...
    def test_classify_hit_and_others(self) -> None:
        self.assertEqual(
            classify_pages_projects(PAGES_TABLE, "roxabi-forge"),
            ("hit", ["client-site"]),
        )
        self.assertEqual(
            classify_pages_projects(PAGES_TABLE, "missing"),
            ("others", ["roxabi-forge", "client-site"]),
        )

    def test_classify_empty_vs_unparsed(self) -> None:
        self.assertEqual(classify_pages_projects("", "roxabi-forge"), ("empty", []))
...
"""
        self.assertEqual(
            classify_pages_projects(header_only, "roxabi-forge"),
            ("unparsed", []),
        )
