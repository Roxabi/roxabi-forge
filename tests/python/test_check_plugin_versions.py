...
CATALOG = {
    ...
    "version": "1.8.1",
    "owner": {"name": "Roxabi"},
    "plugins": [{"name": "roxabi-forge", "version": "1.8.1", "source": "./plugins/roxabi-forge"}],
}
OMP_CATALOG = {
    "name": "roxabi-forge",
    "owner": {"name": "Roxabi"},
...
def _write_tree(base: Path, *, omp_version: str = "1.8.1", changelog: str = CHANGELOG) -> Path:
    (base / "plugins/roxabi-forge/.claude-plugin").mkdir(parents=True)
    (base / "plugins/roxabi-forge/.grok-plugin").mkdir(parents=True)
    (base / "plugins/roxabi-forge/.omp-plugin").mkdir(parents=True)
    (base / "plugins/roxabi-forge/.codex-plugin").mkdir(parents=True)
    (base / ".claude-plugin").mkdir()
    (base / ".grok-plugin").mkdir()
    (base / ".omp-plugin").mkdir()
    for rel in (
        "plugins/roxabi-forge/plugin.json",
    ...
    omp["plugins"] = [{"name": "roxabi-forge", "version": omp_version, "source": "./x"}]
