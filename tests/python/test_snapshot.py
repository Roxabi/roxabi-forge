...
test reads ~/.config/roxabi/forge, touches the real hub, or reaches the network.
"""
...
LIB = ROOT / "plugins" / "roxabi-forge" / "scripts" / "lib"
...
class SnapshotCLIBase(unittest.TestCase):
    ...
    def _write_cfg(self, hub: Path) -> None:
        # This suite is the slug layout: a record keyed on top-level artifact
        # directories. The engine's own data tree is `layout: "tree"` (pages
        # keyed by path, see test_tree_snapshot.py), and the example config
        # merges into every config, so the layout under test has to be stated
        # rather than inferred from the fixture's shape.
        self.cfg_path.write_text(
            json.dumps(
                {"hub_root": str(hub), "artifacts_dir": "artifacts", "layout": "slug"}
            )
            + "\n",
            encoding="utf-8",
        )
...
class LiveDeploymentBranchTests(unittest.TestCase):
    ...
    CFG = {"pages_project": "roxabi-forge"}
