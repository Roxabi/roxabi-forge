...
#   HOME        → temp dir, so ~/.config/roxabi/forge is never the operator's
...
DISCOVER="$ROOT/plugins/roxabi-forge/scripts/forge-discover.sh"
SH="${FORGE_BASH:-bash}"
...
STUB_LIST=projects_none run --project roxabi-forge
...
STUB_LIST=projects_unparsed run --project roxabi-forge
...
echo "$OUT" | grep -qE 'check:|wrangler|forge-doctor\.sh|roxabi-forge-setup' \
...
STUB_WHOAMI_FAIL=1 run --project roxabi-forge
...
STUB_LIST_FAIL=1 run --project roxabi-forge
...
STUB_TOML=nokv run --project roxabi-forge
...
ENVF="$HOME/.config/roxabi/forge/forge.env"
mkdir -p "$HOME/.config/roxabi/forge"
cat > "$ENVF" <<EOF
...
run --project roxabi-forge --write
[ "$RC" -eq 0 ] || fail "--write on a healthy forge must exit 0, got $RC: $OUT"

[ "$(mode_of "$ENVF")" = "-rw-------" ] \
  || fail "forge.env must end up 600, got $(mode_of "$ENVF")"
[ "$(mode_of "$HOME/.config/roxabi/forge")" = "drwx------" ] \
  || fail "\$HOME/.config/roxabi/forge must end up 700, got $(mode_of "$HOME/.config/roxabi/forge")"
...
CFG="$HOME/.config/roxabi/forge/forge.config.json"
cat > "$CFG" <<EOF
{
  "version": 1,
  "hub_root": "$TD/hub",
...
assert cfg.get("pages_project") == "roxabi-forge", cfg.get("pages_project")
...
# These machines symlink ~/.config/roxabi/forge/forge.config.json into a synced
...
# diagnostic and the roxabi-forge-setup skill own creation).
unset FORGE_CONFIG
rm -f "$CFG"
run --project roxabi-forge --write
[ "$RC" -eq 0 ] || fail "--write without a local config must still exit 0, got $RC: $OUT"
[ ! -f "$CFG" ] \
  || fail "--write created $CFG — creation belongs to the roxabi-forge-setup skill"
...
echo "$OUT" | grep -q '/roxabi-forge-setup' \
  || fail "--write without a local config must name /roxabi-forge-setup, got: $OUT"
