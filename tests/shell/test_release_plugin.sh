...
write_version_files() {
  local dest="$1"
  local plugin_json='{"name":"roxabi-forge","version":"9.9.9"}'
  mkdir -p \
    "$dest/scripts" \
    "$dest/plugins/roxabi-forge/.claude-plugin" \
    "$dest/plugins/roxabi-forge/.grok-plugin" \
    "$dest/plugins/roxabi-forge/.omp-plugin" \
    "$dest/plugins/roxabi-forge/.codex-plugin" \
    "$dest/.claude-plugin" \
    "$dest/.grok-plugin" \
    "$dest/.omp-plugin"
  printf '%s\n' "$plugin_json" >"$dest/plugins/roxabi-forge/plugin.json"
  printf '%s\n' "$plugin_json" >"$dest/plugins/roxabi-forge/.claude-plugin/plugin.json"
  printf '%s\n' "$plugin_json" >"$dest/plugins/roxabi-forge/.grok-plugin/plugin.json"
  printf '%s\n' "$plugin_json" >"$dest/plugins/roxabi-forge/.omp-plugin/plugin.json"
  printf '%s\n' "$plugin_json" >"$dest/plugins/roxabi-forge/.codex-plugin/plugin.json"
  printf '%s\n' "$plugin_json" >"$dest/plugins/roxabi-forge/package.json"
  cat >"$dest/.claude-plugin/marketplace.json" <<'EOF'
{
  "name": "roxabi-forge",
  "version": "9.9.9",
  "plugins": [
    {"name": "roxabi-forge", "version": "9.9.9", "source": "./plugins/roxabi-forge"}
  ]
}
EOF
...
tag_ref="refs/tags/roxabi-forge/v9.9.9"
...
if ! git -C "$FIXTURE" ls-remote --tags origin | grep -F -q "refs/tags/roxabi-forge/v9.9.9"; then
...
if ! grep -q "release create roxabi-forge/v9.9.9" "$GH_LOG"; then
  echo "FAIL nominal (gh missing release create roxabi-forge/v9.9.9)"
  cas1=1
fi
