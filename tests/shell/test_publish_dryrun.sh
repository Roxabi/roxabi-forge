...
# shlink are recording PATH stubs. The machine hub, ~/.config/roxabi/forge and the network
...
cd "$ROOT"
PUBLISH="$ROOT/plugins/roxabi-forge/scripts/publish.sh"
...
mkdir -p "$TD/engine/site" "$TD/engine/functions" "$TD/engine/plugins/roxabi-forge"
printf '404\n' > "$TD/engine/site/404.html"
# The doctor refuses a forge_repo without the tree-engine markers (ENGINE_TREE_MARKERS
# in load_config.py), so the fake engine must carry the edge entry point too.
printf 'export {};\n' > "$TD/engine/functions/_middleware.ts"
...
cp -a "$ROOT/plugins/roxabi-forge/scripts" "$TD/engine/plugins/roxabi-forge/scripts" \
