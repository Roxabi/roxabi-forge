...
assert_not_grep \
  'git@github.com:Roxabi/roxabi-forge.git' \
  "$PUBLISH" \
  "publish.sh must not contain the legacy SSH default"

assert_grep \
  'https://github.com/Roxabi/roxabi-forge.git' \
  "$PUBLISH" \
...
engine=$(mktemp -d)
mkdir -p "$engine/site/a" "$engine/functions" "$engine/plugins/roxabi-forge/scripts"
...
if ! (
  ...
  [ -f "$WORK/repo/plugins/roxabi-forge/scripts/x.sh" ] || { echo "missing plugins scripts" >&2; exit 1; }
