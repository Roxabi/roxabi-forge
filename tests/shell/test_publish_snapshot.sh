...
BUILD="$ROOT/plugins/roxabi-forge/scripts/build-site-from-hub.py"
...
# layout: slug — this suite's fixture is the legacy artifact-directory hub, and
# the example config (merged into every config) now declares the tree layout.
cat > "$TD/cfg.json" <<EOF
...
# resolve_api_token() falling back to the operator's real ~/.config/roxabi/forge/forge.env
# — which would hand this suite a production token and point
# `live-deployment` at the live roxabi-forge project. Assert the isolation
...
STRIPLESS="$WORK_STRIPLESS/repo/plugins/roxabi-forge/scripts"
mkdir -p "$STRIPLESS" "$WORK_STRIPLESS/repo/site"
cp -a "$ROOT/plugins/roxabi-forge/scripts/." "$STRIPLESS/"
