...
[![CI](https://github.com/Roxabi/roxabi-forge/actions/workflows/ci.yml/badge.svg)](https://github.com/Roxabi/roxabi-forge/actions/workflows/ci.yml)

**Team HTML artifact host** — [forge.roxabi.dev](https://forge.roxabi.dev)
...
| | |
|---|---|
| **Live** | `https://forge.roxabi.dev` |
| **Audience** | Roxabi team (Access) · external = secret `/s/…` links |
| **Deploy** | Cloudflare Pages Direct Upload (`wrangler`) from your laptop |
| **Plugin** | `roxabi-forge` — `roxabi-forge-publish` · `roxabi-forge-setup` |
...
```
hub / artifacts/<slug>/           SSOT HTML (shared vault, not git)
        │
        ▼  publish.sh
temp tree (functions + site skeleton + hub HTML)
        │
        ▼  wrangler pages deploy
forge.roxabi.dev                 live
```
...
and `chmod 600` on `~/.config/roxabi/forge/forge.env` (the token file is refused when
it is world-readable). Use WSL.

No bash 4 feature and no GNU-only command is allowed in
`plugins/roxabi-forge/scripts/` — `tests/shell/test_bash32_lint.sh` enforces it,
...
One plugin tree (`plugins/roxabi-forge/`). Pick the harness:

> **Upgrading from roxabi-forge 0.x** (plugin `forge@roxabi-forge`, now the
> archived `Roxabi/roxabi-forge-legacy`): the old `forge` plugin is gone from
> the catalog. Uninstall it, refresh the marketplace, then install
> `roxabi-forge@roxabi-forge` as below. Claude Code re-clones the marketplace
> on update even though the history was replaced (verified 2026-09-23):
>
> ```text
> /plugin uninstall forge@roxabi-forge          # Claude Code
> /plugin marketplace update roxabi-forge
> ```
>
> If an update ever refuses the new history, remove the `roxabi-forge`
> marketplace and add it again. Project-scoped installs of `forge` live in
> that project's `.claude/settings.json` and must be uninstalled there.

Skills are `roxabi-forge-publish` and `roxabi-forge-setup`. The `roxabi-`
prefix keeps them apart from another forge plugin that exposes generic
`forge-publish` / `forge-setup` skill names on the same machines.

#### Claude Code

```text
/plugin marketplace add Roxabi/roxabi-forge
/plugin install roxabi-forge@roxabi-forge
```

#### Grok

```bash
grok plugin marketplace add Roxabi/roxabi-forge
grok plugin install roxabi-forge --trust
```

#### Codex

```bash
codex plugin marketplace add Roxabi/roxabi-forge
codex plugin add roxabi-forge@roxabi-forge
```
...
```bash
omp plugin marketplace add Roxabi/roxabi-forge
omp plugin install roxabi-forge@roxabi-forge
```

Then `/reload-plugins` (or restart). Dev on a clone — live symlink:

```bash
omp plugin link ./plugins/roxabi-forge
```

Link is user-global; files stay in the checkout. Do not `omp plugin install github:Roxabi/roxabi-forge` — that installs the engine root, not the plugin.

### Machine setup

Two paths. **Attach** to a forge that already exists (a Roxabi teammate's machine,
...
`--write` merges the discovered keys into `~/.config/roxabi/forge/forge.env` (`chmod 600`,
directory `700`) printing **key names only**, and persists the confirmed
`pages_project` into an **existing** `~/.config/roxabi/forge/forge.config.json` — so
`--project` is needed at most once. It never creates that config file: the
`/roxabi-forge-setup` skill owns creation, and until it runs the project name is not
remembered between calls.

Do **not** `cp .env.example ~/.config/roxabi/forge/forge.env`. The example is a schema
...
|---|---|---|
| `forge-discover.sh` | `roxabi-forge` | attaches to the Roxabi team forge — override with `--project NAME` |
...
A hub without marker directories uses `"vault_markers": []` in
...
```bash
plugins/roxabi-forge/scripts/forge-doctor.sh            # human report
plugins/roxabi-forge/scripts/forge-doctor.sh --json     # payload for agents
plugins/roxabi-forge/scripts/forge-doctor.sh --quiet    # one stderr line if not ready
plugins/roxabi-forge/scripts/forge-doctor.sh --online   # + live Cloudflare checks
```

| Exit | Meaning | Next |
|---|---|---|
| `0` | ready — hub config sound **and** deploy credentials complete | publish |
| `1` | hub / local config KO (`hub_root`, missing keys, no `python3`) | run `/roxabi-forge-setup` |
...
| File | Role |
|---|---|
| `~/.config/roxabi/forge/forge.config.json` | `hub_root`, `pages_project`, `public_host`, `vault_markers` (local, not git) |
| `~/.config/roxabi/forge/forge.env` | CF credentials + the Pages plain vars injected at deploy · `chmod 600` |
...
doctor issues and names `/roxabi-forge-setup` rather than deploying with example
...
plugins/roxabi-forge/          # multi-harness plugin (publish + setup)
...
| Doc | Topic |
|---|---|
| [docs/cloudflare-access.md](docs/cloudflare-access.md) | Access + Bypass — **Roxabi team-prod instance (forge.roxabi.dev)** |
| [docs/cloudflare-pages.md](docs/cloudflare-pages.md) | Deploy + Pages env — **Roxabi team-prod instance (forge.roxabi.dev)** |
