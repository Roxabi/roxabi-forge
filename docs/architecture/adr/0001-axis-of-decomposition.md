...
roxabi-forge varies along four dimensions at once, and only one of them can own
the code structure:
...
| **C — plane** | publisher CLI (bash + Python) · edge (Pages Functions + KV) · hub SSOT (shared hub, outside git) | 0 (fixed) |
...
grep -rn 'share-bar\.js' plugins/roxabi-forge/scripts/
```
