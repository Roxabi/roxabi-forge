# Cloudflare Access — Roxabi team-prod instance (forge.roxabi.dev)

**Scope:** the **Roxabi** production forge (`forge.roxabi.dev`, Pages project
`forge`, tree layout). Configuring your own forge? Do not create these apps by
hand and do not point them at our host — `plugins/roxabi-forge/scripts/forge-provision.sh`
creates the applications on your account, in the safe order, with your host.
Client-owned setup notes live in [artifacts-config.md](./artifacts-config.md).

## Goal

| Path | Who |
|---|---|
| `/` catalogue shell · `/<tree path>.html` (legacy `/a/<slug>/`) | Team (Access JWT) unless visibility is **public** |
| `/s/<id>/<key>/` | Anyone with the secret link (exchanged for a cookie) |
...
| App | Domain path | Policy | On `forge.roxabi.dev` |
|---|---|---|---|
| **Roxabi Forge · login** | `forge.roxabi.dev/login` | Allow team emails / IdP group | deployed |
| public surface | host `/`, pages, `/s/*`, `/api/*` | **Bypass** everyone (Functions enforce visibility) | **not deployed** — the host is not behind Access at all, so there is nothing to bypass; Functions alone decide |
| pages.dev | `<project>.pages.dev` | Allow team | **not deployed** — the middleware answers 403 on every path of the preview origin |

`forge-provision.sh` creates all three on a fresh account, where the whole host
may already sit behind an Access application; the Roxabi instance only needs the
login app.
...
|---|---|---|
| `forge.roxabi.dev` | Worker visibility | Function + KV |
...
```bash
curl -sI "https://forge.roxabi.dev/login"                 # 302 → Access
curl -sI "https://forge.roxabi.dev/a/<private-slug>/"      # 302 without cookie
curl -sI "https://forge.roxabi.dev/s/<slug>/<key>/"        # 200 without cookie if key valid
